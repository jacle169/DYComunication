...
namespace DYComWinClient
{
    ...
    public class ClientPoxy: IDisposable 
    {
        ...
        public ClientPoxy(string ServerIP, int Port, int BufferCount)
        {
            ...
            {
                ReceiveEventArgs.RemoteEndPoint = new IPEndPoint(IPAddress.Parse(ServerIP), Port);
            }
            else
            {
                IPHostEntry ip = Dns.Resolve(ServerIP);
                ReceiveEventArgs.RemoteEndPoint = new IPEndPoint(ip.AddressList[0], Port);
            }
